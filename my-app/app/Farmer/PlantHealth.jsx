...
const handleEditPlant = (plant) => {
  ...
  const handleUpdateState = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}/updatePlant/${plantId}`, {
        ...
        body: JSON.stringify({
          lastAction: plantState.action,
          lastActionAmount: plantState.amount,
          lastActionDate: plantState.date,
          notes: plantState.notes
        }),
      });
      ...
      Alert.alert('Success', 'Plant maintenance updated successfully');
      setShowUpdateStateModal(false);
      refreshPlants();
    } catch (error) {
      console.error('Error updating plant state:', error);
      Alert.alert('Error', 'Failed to update plant maintenance. Please try again.');
    }
  };
  ...
  const renderPlantCard = (plant) => (
    <View key={`${plant.Name}-${plant.Greenhouse}`} style={styles.card}>
      ...
      <View style={styles.cardActions}>
        ...
        >
          <Text style={styles.updateStateButtonText}>Update Maintenance</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  const renderUpdateStateModal = () => (
    <Modal
      visible={showUpdateStateModal}
      animationType="slide"
      transparent={true}
      onRequestClose={() => setShowUpdateStateModal(false)}
    >
      <View style={styles.modalOverlay}>
        <ScrollView>
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Update Plant Maintenance</Text>
              <TouchableOpacity
                onPress={() => setShowUpdateStateModal(false)}
                style={styles.closeButton}
              >
                <Icon name="close" size={24} color="#666" />
              </TouchableOpacity>
            ...
            <View style={styles.formGroup}>
              <Text style={styles.label}>Maintenance Action</Text>
              <View style={styles.actionOptions}>
                <TouchableOpacity
                  style={[
                    styles.actionOption,
                    plantState.action === 'watering' && styles.actionOptionSelected
                  ]}
                  onPress={() => setPlantState(prev => ({ ...prev, action: 'watering' }))}
                >
                  <Icon name="water" size={24} color={plantState.action === 'watering' ? '#fff' : '#0d986a'} />
                  <Text style={[
                    styles.actionOptionText,
                    plantState.action === 'watering' && styles.actionOptionTextSelected
                  ]}>Watering</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.actionOption,
                    plantState.action === 'fertilizing' && styles.actionOptionSelected
                  ]}
                  onPress={() => setPlantState(prev => ({ ...prev, action: 'fertilizing' }))}
                >
                  <Icon name="sprout" size={24} color={plantState.action === 'fertilizing' ? '#fff' : '#0d986a'} />
                  <Text style={[
                    styles.actionOptionText,
                    plantState.action === 'fertilizing' && styles.actionOptionTextSelected
                  ]}>Fertilizing</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[
                    styles.actionOption,
                    plantState.action === 'pruning' && styles.actionOptionSelected
                  ]}
                  onPress={() => setPlantState(prev => ({ ...prev, action: 'pruning' }))}
                >
                  <Icon name="content-cut" size={24} color={plantState.action === 'pruning' ? '#fff' : '#0d986a'} />
                  <Text style={[
                    styles.actionOptionText,
                    plantState.action === 'pruning' && styles.actionOptionTextSelected
                  ]}>Pruning</Text>
                </TouchableOpacity>
              </View>
            </View>
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                style={styles.input}
                placeholder={plantState.action === 'watering' ? "Enter water amount (L)" : 
                           plantState.action === 'fertilizing' ? "Enter fertilizer amount (g)" : 
                           "Enter details"}
                value={plantState.amount}
                onChangeText={(text) => setPlantState(prev => ({ ...prev, amount: text }))}
            ...
            <View style={styles.formGroup}>
              ...
              >
                <Icon name="calendar" size={20} color="#0d986a" />
                <Text style={styles.datePickerText}>
                  {new Date(plantState.date).toLocaleDateString()}
                </Text>
              </TouchableOpacity>
              {showDatePicker && (
                <DateTimePicker
                  testID="dateTimePicker"
                  value={new Date(plantState.date)}
                  mode="date"
                  is24Hour={true}
            ...
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                multiline
                numberOfLines={4}
                value={plantState.notes}
                onChangeText={(text) => setPlantState(prev => ({ ...prev, notes: text }))}
              />
            </View>
            ...
            >
              <Text style={styles.submitButtonText}>Update Maintenance</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      </View>
