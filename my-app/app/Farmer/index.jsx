...
const WeatherCard = ({ defaultLocation = "London" }) => {
  ...
  const getDeviceLocation = async () => {
    try {
      ...
      setLocation(defaultLocation);
      // Try to fetch weather for default location
  ...
  const fetchWeatherData = async (locationToUse) => {
    if (!locationToUse) {
      console.log('No location provided for weather fetch');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${baseUrl}/weather?location=${encodeURIComponent(locationToUse)}`);
      ...
      Alert.alert(
        'Weather Error',
        error.message || 'Failed to load weather data. Please try again later.',
        [{ text: 'OK' }]
      );
    } finally {
      setLoading(false);
